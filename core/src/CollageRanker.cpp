...
CollageRanker::CollageRanker(const Config &config)
{
    ...
    try 
    {
        bias = torch::tensor(KeywordRanker::parse_float_vector("extractor/models/w2vv-img_bias-2048floats.bin", 2048));
    }
    catch (const c10::Error& e) 
    {
    ...
    try 
    {
        weights = torch::tensor(KeywordRanker::parse_float_vector("extractor/models/w2vv-img_weight-2048x4096floats.bin", 4096*2048)).reshape({2048, 4096}).permute({1, 0});
    }
    catch (const c10::Error& e) 
    {
